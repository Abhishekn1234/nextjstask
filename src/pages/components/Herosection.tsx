...
export default function HeroSection({ hero }: { hero: LandingContent["hero"] }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const ctx = gsap.context(() => {
        ...
        if (phoneRef.current)
          tl.from(phoneRef.current, { y: 50, opacity: 0, duration: 1 });

        if (!isMobile) {
          if (leftCardRef.current)
            tl.from(
              leftCardRef.current,
              { x: -50, opacity: 0, duration: 0.8, ease: "back.out(1.2)" },
              "-=0.6"
            );
  ...
  return (
    <section
      ref={sectionRef}
      className={`min-h-screen flex flex-col items-center font-inter
        ${isMobile
          ? "px-4 pt-16 pb-24 bg-gradient-to-b from-white to-blue-50"
          : "px-8 sm:px-10 lg:px-20 pt-24 pb-40 bg-gradient-to-b from-white to-blue-50"
        }`}
    >
      ...
      <div className="text-center max-w-xl mx-auto px-2 sm:px-0">
        <h1 className="hero-animate font-extrabold text-black leading-tight mb-5 text-3xl sm:text-4xl md:text-5xl lg:text-6xl">
          {hero.title}
        </h1>
        <p className="hero-animate text-black mb-8 text-base sm:text-lg md:text-xl">
          {hero.subtitle}
        </p>

        <div className="hero-animate flex flex-col sm:flex-row justify-center gap-4">
          <button className="px-6 py-3 rounded-full bg-blue-100 text-black font-semibold hover:bg-blue-600 transition-colors duration-300 w-full sm:w-auto">
             App Store
          </button>
          <button className="px-6 py-3 rounded-full bg-gray-100 text-black font-semibold hover:bg-green-600 transition-colors duration-300 w-full sm:w-auto">
            ▶ Google Play
          </button>
        </div>
      </div>
